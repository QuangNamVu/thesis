...
from info_params import IS_CONV_l2
...
@add_arg_scope
def conv1d(name, inputs, kernel_size, in_C, out_C, stride=1, padding='SAME', data_format='NWC',
           init_scale=0.1, init=False, mask=None, dtype=tf.float32, reuse=False, **_):
    with tf.variable_scope(name, reuse=reuse):
        # [filter_width, in_channels, out_channels]
        filter_shape = [kernel_size, in_C, out_C]

        if init:
            v = tf.get_variable("V", shape=filter_shape, dtype=dtype,
                                # initializer=tf.contrib.layers.xavier_initializer())
                                # initializer=tf.random_normal_initializer(0, 0.05, dtype=dtype)
            ...
            if IS_CONV_l2 is True:
                v_norm = tf.nn.l2_normalize(v, [0, 1])
            else:
                v_norm = v
            # [M, W, in_C] => [M, W, out_C]
            x_init = tf.nn.conv1d(value=inputs, filters=v_norm, stride=stride, padding=padding, data_format=data_format)

            # mean ,variance in [out_C]
            m_init, v_init = tf.nn.moments(x_init, [0, 1])

            scale_init = init_scale / tf.sqrt(v_init + 1e-10)
            _ = tf.get_variable("g_" + name, initializer=tf.log(scale_init) / 3.0)
            _ = tf.get_variable("b_" + name, initializer=-m_init * scale_init)
            return tf.reshape(scale_init, [1, -1, 1, 1]) * (x_init - tf.reshape(m_init, [1, -1, 1, 1]))
        else:

            v = tf.get_variable("V_" + name, filter_shape, initializer=tf.keras.initializers.lecun_uniform(seed=None))

            g = tf.get_variable("g_" + name, [out_C], initializer=tf.keras.initializers.lecun_uniform(seed=None))
            b = tf.get_variable("b_" + name, [out_C], initializer=tf.keras.initializers.lecun_uniform(seed=None))

            if mask is not None:
                v = mask * v

            # use weight normalization (Salimans & Kingma, 2016)
            w = tf.reshape(tf.exp(g), [1, 1, out_C]) * tf.nn.l2_normalize(v, [0, 1, 2])

            return tf.nn.conv1d(value=inputs, filters=w, stride=stride, padding=padding, data_format=data_format) + b


@add_arg_scope
def ar_conv1d(name, value, kernel_size, in_C, out_C, stride=1, padding="SAME", data_format='NWC', init_scale=.1,
              zerodiagonal=True, **_):
    np_mask_1d = np.squeeze(get_conv_ar_mask(kernel_size, w=1, n_in=in_C, n_out=out_C, zerodiagonal=zerodiagonal),
                            axis=1)

    mask = tf.constant(np_mask_1d)

    with arg_scope([conv1d]):
        # return conv1d(name, value=value, kernel_size=kernel_size, in_C=in_C, out_C=out_C, stride=stride,
        #               padding=padding, data_format=data_format, init_scale=init_scale, mask=mask)

        return conv1d(name=name, inputs=value, kernel_size=kernel_size, in_C=in_C, out_C=out_C, stride=1,
                      padding=padding, data_format=data_format, init_scale=init_scale, mask=mask)
...
@add_arg_scope
def ar_conv2d(name, x, num_filters, filter_size=(3, 3), stride=(1, 1), pad="SAME", init_scale=1., zerodiagonal=True,
              **_):
    h = filter_size[0]
    w = filter_size[1]
