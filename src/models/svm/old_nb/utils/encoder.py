...
def encoder(self, x):
    ...
    # [M, T, D] => [M, T, f0]
    conv_l1 = conv1d(name='encode_l1', inputs=x, kernel_size=self.hps.lst_kernels[0],
    ...
    activate_l1 = tf.nn.elu(batch_l1)
    max_pool_l1 = tf.layers.max_pooling1d(inputs=activate_l1, pool_size=2, strides=1,
                                          padding='SAME', data_format='channels_last')
    out_l1 = tf.layers.dropout(inputs=max_pool_l1, rate=self.hps.dropout_rate, training=is_training)

    # [M, T, f0] => [M, T, f1]
    conv_l2 = conv1d(name='encode_l2', inputs=out_l1, kernel_size=self.hps.lst_kernels[1],
                     in_C=self.hps.f[0], out_C=self.hps.f[1])
