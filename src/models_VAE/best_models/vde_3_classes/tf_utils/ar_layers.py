...
def conv1d(name, inputs, kernel_size, in_C, out_C, stride=1, padding='SAME', data_format='NWC',
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        if init:
            ...
            v = tf.get_variable("V_" + name, filter_shape, initializer=tf.contrib.layers.xavier_initializer())

            g = tf.get_variable("g_" + name, [out_C], initializer=tf.contrib.layers.xavier_initializer())
            b = tf.get_variable("b_" + name, [out_C], initializer=tf.contrib.layers.xavier_initializer())
