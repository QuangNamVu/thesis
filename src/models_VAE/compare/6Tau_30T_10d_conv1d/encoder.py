...
def encoder(self, x):
    ...
    conv_l2 = conv1d(name='encode_l2', inputs=out_l1, kernel_size=self.hps.lst_kernels[1], in_C=self.hps.f[0],
                     out_C=self.hps.f[1])
    activate_l2 = tf.nn.tanh(conv_l2)
    ...
    if self.hps.is_VDE:
        z_lst = gaussian_dense(name='encode_l3', inputs=next_seq, out_C=2 * self.hps.n_z)
    else:
        rs_l3 = tf.reshape(out_l2, [-1, self.hps.T * self.hps.f[1]])
        z_lst = gaussian_dense(name='encode_l3', inputs=rs_l3, out_C=2 * self.hps.n_z)
