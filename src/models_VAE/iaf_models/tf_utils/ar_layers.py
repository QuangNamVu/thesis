...
@add_arg_scope
def conv1d(name, value, kernel_size, in_C, out_C, stride=1, padding='SAME', data_format='NWC',
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        if init:
            ...
            # [M, W, in_C] => [M, W, out_C]
            x_init = tf.nn.conv1d(value=value, filters=v_norm, stride=stride, padding=padding, data_format=data_format)
            ...
            v = tf.get_variable("V_" + name, filter_shape)
            g = tf.get_variable("g_" + name, [out_C])
            b = tf.get_variable("b_" + name, [out_C])
            ...
            return tf.nn.conv1d(value=value, filters=w, stride=stride, padding=padding, data_format=data_format) + b
