...
def get_data(hps):
    ...
    segment, next_segment, target_one_hot = segment_seq(
        dfX, df_next_deltaClose, hps)
    ...
    # X constructed = X tau
    if hps.is_VDE:
        train_data = LoadData(train_segment, train_next_shift,
                              train_target_one_hot, shuffle=True)
        test_data = LoadData(test_segment, test_next_shift,
                             test_target_one_hot, shuffle=False)

    else:
        # X constructed = X
        train_data = LoadData(train_segment, train_segment,
                              train_target_one_hot, shuffle=True)
        test_data = LoadData(test_segment, test_segment,
                             test_target_one_hot, shuffle=False)
